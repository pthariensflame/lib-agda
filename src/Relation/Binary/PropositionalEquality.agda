...
subst-eq : ∀ {i j} {A : Set i} {B : A → Set j} (f : (v : A) → B v) {x : A} {y : A} (eq : x ≡ y) → f y ≡ subst B eq (f x)
subst-eq f refl = refl

subst₂-eq : ∀ {i₁ i₂ j} {A₁ : Set i₁} {A₂ : Set i₂} {B : A₁ → A₂ → Set j} (f : (v₁ : A₁) → (v₂ : A₂) → B v₁ v₂) {x₁ : A₁} {y₁ : A₁} (eq₁ : x₁ ≡ y₁) {x₂ : A₂} {y₂ : A₂} (eq₂ : x₂ ≡ y₂) → f y₁ y₂ ≡ subst₂ B eq₁ eq₂ (f x₁ x₂)
subst₂-eq f refl refl = refl
