...
-- You can "post-compose" an indexed setoid with a function on the index

_on_ : ∀ {i j s₁ s₂} {I : Set i} {J : Set j} → Setoid I s₁ s₂ → (J → I) → Setoid J _ _
S on f = record
  { Carrier       = λ x → S.Carrier (f x)
  ; _≈_           = S._≈_
  ; isEquivalence = record
    { refl  = S.refl
    ; sym   = S.sym
