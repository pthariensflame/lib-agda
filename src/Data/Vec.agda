...
headTail : ∀ {a n} {A : Set a} → Vec A (suc n) → A × Vec A n
headTail (x ∷ xs) = x , xs

head : ∀ {a n} {A : Set a} → Vec A (suc n) → A
head = Prod.proj₁ ∘′ headTail

tail : ∀ {a n} {A : Set a} → Vec A (suc n) → Vec A n
tail = Prod.proj₂ ∘′ headTail
...
initLast : ∀ {a n} {A : Set a} (xs : Vec A (suc n)) →
...
init : ∀ {a n} {A : Set a} → Vec A (suc n) → Vec A n
init xs         with initLast xs
init .(ys ∷ʳ y) | (ys , y , refl) = ys

last : ∀ {a n} {A : Set a} → Vec A (suc n) → A
